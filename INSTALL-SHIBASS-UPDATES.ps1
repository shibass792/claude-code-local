...
.EXAMPLE
  ...
  Or double-click: INSTALL-SHIBASS-UPDATES.cmd
#>
param(
...
function Copy-RepoToTarget([string]$SourceRoot) {
  ...
  if (Test-Path $portsSrc) {
    Copy-Item -Path "$portsSrc\*" -Destination (Join-Path $TargetRoot "config") -Force
    Write-Host "  Config -> config\" -ForegroundColor Green
  }

  Ensure-Dir (Join-Path $TargetRoot "10_OUTPUTS\social")

  foreach ($file in @("INSTALL-SHIBASS-UPDATES.ps1", "INSTALL-SHIBASS-UPDATES.cmd", "START-SOCIAL-STUDIO.cmd")) {
...
try {
  $zipPath = Join-Path $env:TEMP "shibass-$($Branch.Replace('/','-')).zip"
  ...
  Write-Host "ZIP failed ($($_.Exception.Message)). Trying git clone..." -ForegroundColor Yellow
  ...
  $extractedRoot = Get-ChildItem $staging -Directory | Select-Object -First 1
  if (-not $extractedRoot) { throw "clone folder empty" }
  $sourceRoot = $extractedRoot.FullName
}
...
if (-not $SkipNpmInstall) {
  ...
  if ((Test-Path $pp) -and (Get-Command npm -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "  SKIP npm — install Node.js or run: cd promo-publisher && npm install" -ForegroundColor Yellow
  }
}
...
Write-Host ""
Write-Host "Next steps on your PC:" -ForegroundColor White
