...
#
# Generated by setup.sh. Server startup, model-mismatch restarts and the
# health-check timeout all live in the repo's shared launcher lib, so fixes
# there apply here too instead of drifting from a copy baked into this file.
LIB="$SCRIPT_DIR/launchers/lib/claude-local-common.sh"
if [ ! -f "\$LIB" ]; then
  echo "ERROR: \$LIB is missing."
  echo "The claude-code-local checkout moved or was deleted — re-run setup.sh from its new location."
  exit 1
fi

export MLX_PYTHON="$MLX_VENV/bin/python3"
export MLX_SERVER="$SERVER_DIR/server.py"
source "\$LIB"

CLAUDE_BIN="\${CLAUDE_BIN:-\$(command -v claude || echo "$CLAUDE_BIN")}"

ensure_mlx_server "$MODEL_ID" "  Loading $MODEL_LABEL on MLX..."
